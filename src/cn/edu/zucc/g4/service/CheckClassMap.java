...
public class CheckClassMap {
	...
	int[][] edges;//邻接表数组
	@Resource(name = "SelectionRecordDAO")
	SelectionRecordDAO srdao;
    ...
    {
		stuList = (ArrayList<StudentBean>) studao.listAllStudent();
        ...
        for(int i=0;i<stuList.size();i++) {//遍历所有学生的课程
        	ArrayList<SelectionRecordBean> templist = (ArrayList<SelectionRecordBean>) srdao.getCourseByStudentId(stuList.get(i).getStudent_id());//通过学生id查找课程
        	for(int j = 0;j<templist.size();j++) {//遍历此学生的所有课程
        		int nowindex;
        		nowindex =csmap.get(templist.get(j).getCourse_id());//获取课程下标
        		for(int m = 0;m<templist.size()-1;m++) {//遍历该学生此课程之后的课程并将其链接
        			edges[nowindex][m]=1;
        			edges[m][nowindex]=1;
        		}

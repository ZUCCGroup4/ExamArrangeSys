package cn.edu.zucc.g4.bean;

public class SelectionDetailBean {

	private int selection_detail_id;
	private String student_id;
	private String selection_id;
	public int getSelection_detail_id() {
		return selection_detail_id;
	}
	public void setSelection_detail_id(int selection_detail_id) {
		this.selection_detail_id = selection_detail_id;
	}
	public String getStudent_id() {
		return student_id;
	}
	public void setStudent_id(String student_id) {
		this.student_id = student_id;
	}
	public String getSelection_id() {
		return selection_id;
	}
	public void setSelection_id(String selection_id) {
		this.selection_id = selection_id;
	}
}
